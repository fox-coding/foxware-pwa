<template>
  <div>
      ...
      <div v-if="numberOfPages > 1">
        ...
        <slot name="bullets" v-bind="{ numberOfPages, page, go }">
          <SfBullets
            :total="numberOfPages"
            :current="page - 1"
            :class="'absolute left-0 right-0 m-auto justify-center items-center'"
    ...
    <div class="absolute w-1/5 h-full left-0 top-0 overflow-auto">
      <div
        class="w-24 h-24 border border-gray-300 rounded-sm m-2"
        v-for="(slide, index) in slides"
        :key="slide.mediaId"
        :style="{
          background: `url(${slide.mediaUrl})`,
